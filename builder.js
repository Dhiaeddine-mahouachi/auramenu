(() => {
  const i18n = {
    tr: {
      ...
      address: "Adres",
      businessPhone: "Müşterilerin göreceği telefon",
      hours: "Çalışma saatleri",
      template: "Tasarım",
      menuLanguage: "Müşteri menüsünün dili",
      ...
      email: "E-posta",
      city: "Şehir",
      paymentRef: "Ödeme notu / referansı",
      notes: "AuraDigital için notunuz",
      consent:
        "Gönderdiğiniz bilgiler AuraDigital panelinde kırmızı bir talep oluşturur. Menü, ödeme kontrolü ve sizin izniniz olmadan yayınlanmaz.",
      ...
      product: "Ürün adı",
      price: "Fiyat",
      image: "İsterseniz görsel bağlantısı (URL)",
      photo: "Ürün fotoğrafı",
      choosePhoto: "Fotoğraf seç",
      changePhoto: "Fotoğrafı değiştir",
      removePhoto: "Fotoğrafı kaldır",
      photoHelp:
        "JPG, PNG veya WebP · otomatik küçültülür · en fazla 12 · sayfadan ayrılmadan gönderin",
      ...
      delete: "Sil",
      required: "İşletme adı, özel adres, yetkili kişi ve telefon zorunludur.",
      menuRequired: "En az bir kategori ve bir ürün ekleyin.",
      network:
        "Talep gönderilemedi. İnternet bağlantınızı kontrol edip tekrar deneyin.",
    },
    en: {
      ...
      address: "Address",
      businessPhone: "Public business phone",
      hours: "Opening hours",
      template: "Design",
      menuLanguage: "Customer menu language",
      currency: "Currency",
      ...
      email: "Email",
      city: "City",
      paymentRef: "Payment note / reference",
      notes: "Note for AuraDigital",
      consent:
        "Your submission creates a red request in AuraDigital. It will not be published without payment verification and your permission.",
      ...
      price: "Price",
      image: "Optional image link (URL)",
      photo: "Product photo",
      choosePhoto: "Choose photo",
      changePhoto: "Change photo",
      removePhoto: "Remove photo",
      photoHelp:
        "JPG, PNG or WebP · resized automatically · up to 12 · submit before leaving",
      ...
      delete: "Delete",
      required:
        "Business name, custom URL, contact person and phone are required.",
      menuRequired: "Add at least one category and one product.",
      network:
    ...
    ar: {
      ...
      address: "العنوان",
      businessPhone: "هاتف النشاط الظاهر للعملاء",
      hours: "ساعات العمل",
      template: "التصميم",
      menuLanguage: "لغة قائمة العملاء",
      currency: "العملة",
      ...
      city: "المدينة",
      paymentRef: "ملاحظة / مرجع الدفع",
      notes: "ملاحظة إلى AuraDigital",
      consent:
        "يُنشئ الإرسال طلباً أحمر في لوحة AuraDigital. لن تُنشر القائمة دون التحقق من الدفع وموافقتك.",
      submit: "إرسال الطلب",
      ...
      product: "اسم المنتج",
      price: "السعر",
      image: "رابط صورة اختياري (URL)",
      photo: "صورة المنتج",
      choosePhoto: "اختر صورة",
      changePhoto: "غيّر الصورة",
      removePhoto: "احذف الصورة",
      photoHelp:
        "JPG أو PNG أو WebP · تُصغّر تلقائياً · حتى 12 · أرسل قبل مغادرة الصفحة",
      ...
      delete: "حذف",
      required: "اسم النشاط والرابط والشخص المسؤول ورقم الهاتف مطلوبة.",
      menuRequired: "أضف قسماً واحداً ومنتجاً واحداً على الأقل.",
      network: "تعذر إرسال الطلب. تحقق من الاتصال وحاول مرة أخرى.",
    },
  };
  const templateNames = {
    modern: "Nova Mobile",
            ...
            items: [
              {
                ...
                name: "برجر الشيف",
                price: "320",
                description: "لحم مشوي، جبن وصوص خاص",
                imageUrl: "",
                featured: true,
              },
            ],
            ...
            items: [
              {
                ...
                name: "سلطة موسمية",
                price: "180",
                description: "خضار طازجة وصوص الرمان",
                imageUrl: "",
                featured: false,
              },
            ],
              ...
              items: [
                {
                  ...
                  name: "Chef Burger",
                  price: "320",
                  description: "Grilled beef, cheese and signature sauce",
                  imageUrl: "",
                  featured: true,
                },
              ],
              ...
              items: [
                {
                  ...
                  name: "Seasonal Salad",
                  price: "180",
                  description: "Fresh greens and pomegranate dressing",
                  imageUrl: "",
                  featured: false,
                },
              ],
              ...
              items: [
                {
                  ...
                  name: "Şef Burger",
                  price: "320",
                  description: "Izgara dana, peynir ve özel sos",
                  imageUrl: "",
                  featured: true,
                },
              ],
              ...
              items: [
                {
                  ...
                  name: "Mevsim Salata",
                  price: "180",
                  description: "Taze yeşillik ve nar ekşisi",
                  imageUrl: "",
                  featured: false,
                },
              ],
  ...
  try {
    const saved = JSON.parse(localStorage.getItem("auraMenuDraft") || "null");
  ...
  function imageSource(item) {
    if (
      ...
      /^data:image\/(?:jpeg|png|webp);base64,/i.test(item.imageData)
    ) {
  ...
  function uploadedPhotoCount() {
    return categories.reduce(
      (total, category) =>
        total + category.items.filter((item) => Boolean(item.imageData)).length,
      0,
    );
  }
  ...
  async function compressPhoto(file) {
    if (!PHOTO_TYPES.has(file.type)) throw new Error(i18n[lang].invalidPhoto);
    if (file.size > MAX_SOURCE_PHOTO_BYTES)
      throw new Error(i18n[lang].sourceTooLarge);
    const photo = await loadPhoto(file);
    try {
      if (!photo.width || !photo.height) throw new Error(i18n[lang].invalidPhoto);
      let scale = Math.min(1, 960 / Math.max(photo.width, photo.height));
      let quality = 0.82;
      for (let attempt = 0; attempt < 8; attempt += 1) {
        ...
        if (!context) throw new Error(i18n[lang].photoTooLarge);
        context.drawImage(photo.source, 0, 0, canvas.width, canvas.height);
        const blob = await canvasToBlob(canvas, "image/webp", quality);
        if (
          blob &&
          PHOTO_TYPES.has(blob.type) &&
          blob.size <= MAX_PHOTO_BYTES
        ) {
          return blobToDataUrl(blob);
        }
        quality = Math.max(0.5, quality - 0.08);
        if (attempt >= 3) scale *= 0.84;
      }
      throw new Error(i18n[lang].photoTooLarge);
    } finally {
      photo.close();
    }
  }
  function applyLanguage(code) {
    lang = code;
    localStorage.setItem("auraMenuUiLanguage", code);
    document.documentElement.lang = code;
    document.documentElement.dir = code === "ar" ? "rtl" : "ltr";
    document.body.dir = code === "ar" ? "rtl" : "ltr";
    document.querySelectorAll("[data-i18n]").forEach((element) => {
      if (i18n[code][element.dataset.i18n])
        element.textContent = i18n[code][element.dataset.i18n];
    });
    document
  ...
  function renderEditors() {
    $("categoryList").innerHTML = categories
      .map(
        (category) => `<article class="category-editor" data-category="${category.id}">
          <div class="category-row">
            <input value="${esc(category.emoji)}" data-category-field="emoji" maxlength="12" aria-label="Emoji">
            <input value="${esc(category.name)}" data-category-field="name" maxlength="80" placeholder="${esc(i18n[lang].category)}">
            <button class="icon-button" data-remove-category type="button" title="${esc(i18n[lang].delete)}">×</button>
          </div>
          <div class="items-editor">${category.items
            .map((item) => {
              const source = imageSource(item);
              return `<div class="item-editor" data-item="${item.id}">
                <input value="${esc(item.name)}" data-item-field="name" maxlength="100" placeholder="${esc(i18n[lang].product)}">
                <input value="${esc(item.price)}" data-item-field="price" maxlength="40" placeholder="${esc(i18n[lang].price)}">
                <input value="${esc(item.imageUrl || "")}" data-item-field="imageUrl" maxlength="800" placeholder="${esc(i18n[lang].image)}">
                <button class="icon-button" data-remove-item type="button" title="${esc(i18n[lang].delete)}">×</button>
                <textarea data-item-field="description" maxlength="300" placeholder="${esc(i18n[lang].productDescription)}">${esc(item.description)}</textarea>
                <div class="item-photo">
                  <div class="photo-thumb ${source ? "has-photo" : ""}">${source ? `<img src="${esc(source)}" alt="">` : "<span>📷</span>"}</div>
                  <div class="photo-copy"><strong>${esc(i18n[lang].photo)}</strong><small data-photo-status>${esc(item.imageData ? i18n[lang].photoReady : i18n[lang].photoHelp)}</small></div>
                  <div class="photo-actions">
                    <label class="photo-button"><input type="file" data-photo-input accept="image/jpeg,image/png,image/webp"><span>${esc(source ? i18n[lang].changePhoto : i18n[lang].choosePhoto)}</span></label>
                    ${item.imageData ? `<button class="photo-remove" data-remove-photo type="button">${esc(i18n[lang].removePhoto)}</button>` : ""}
                  </div>
                </div>
                <label class="item-featured"><input type="checkbox" data-item-field="featured" ${item.featured ? "checked" : ""}> ${esc(i18n[lang].featured)}</label>
              </div>`;
            })
            .join("")}</div>
          <button class="add-button" data-add-product type="button">${esc(i18n[lang].addProduct)}</button>
        </article>`,
      )
      .join("");
    bindEditorEvents();
  ...
  function bindEditorEvents() {
    document.querySelectorAll("[data-category]").forEach((editor) => {
      const category = categories.find(
        (value) => value.id === editor.dataset.category,
      );
      editor.querySelectorAll("[data-category-field]").forEach((input) =>
        input.addEventListener("input", () => {
          category[input.dataset.categoryField] = input.value;
          updatePreview();
          saveDraft();
        }),
      );
      editor
        .querySelector("[data-remove-category]")
        .addEventListener("click", () => {
          categories = categories.filter((value) => value.id !== category.id);
          renderEditors();
        });
      editor
        .querySelector("[data-add-product]")
        .addEventListener("click", () => {
          if (category.items.length >= 20) return;
          category.items.push({
            id: uid(),
            name: "",
            price: "",
      ...
      editor.querySelectorAll("[data-item]").forEach((itemEditor) => {
        const item = category.items.find(
          (value) => value.id === itemEditor.dataset.item,
        );
        itemEditor.querySelectorAll("[data-item-field]").forEach((input) =>
          input.addEventListener("input", () => {
            item[input.dataset.itemField] =
        ...
        itemEditor
          ...
          .addEventListener("change", async (event) => {
            ...
            if (!file) return;
            if (!item.imageData && uploadedPhotoCount() >= MAX_PHOTOS) {
              showMessage(i18n[lang].photoLimit);
              event.target.value = "";
              return;
            }
            const status = itemEditor.querySelector("[data-photo-status]");
            event.target.disabled = true;
            status.textContent = i18n[lang].photoProcessing;
            try {
              item.imageData = await compressPhoto(file);
              renderEditors();
            } catch (error) {
              showMessage(error.message || i18n[lang].invalidPhoto);
              status.textContent = i18n[lang].photoHelp;
              event.target.disabled = false;
              event.target.value = "";
            }
          });
        itemEditor
          .querySelector("[data-remove-photo]")
          ?.addEventListener("click", () => {
            item.imageData = "";
            renderEditors();
          });
  ...
  function saveDraft() {
    const draft = {
      categories: categories.map((category) => ({
        ...category,
        items: category.items.map(({ imageData, ...item }) => item),
      })),
      fields: {},
    };
    [
    ...
    localStorage.setItem("auraMenuDraft", JSON.stringify(draft));
  }
  function restoreDraft() {
    try {
      const draft = JSON.parse(localStorage.getItem("auraMenuDraft") || "null");
  ...
  $("addCategory").addEventListener("click", () => {
    ...
    categories.push({ id: uid(), emoji: "🍽️", name: "", items: [] });
    renderEditors();
  });
  [
    "businessName",
  ...
  $("businessName").addEventListener("blur", () => {
    if (!$("slug").dataset.edited) {
      $("slug").value = normalizeSlug($("businessName").value);
      updatePreview();
      saveDraft();
    }
  });
  ...
  $("menuForm").addEventListener("submit", async (event) => {
    ...
    try {
      ...
      showMessage(error.message || i18n[lang].network);
    } finally {
      button.disabled = false;
      button.querySelector("span").textContent = i18n[lang].submit;
    }
  });
  restoreDraft();
